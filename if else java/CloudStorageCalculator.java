//20  Cloud Storage Subscription Calculator
...
public class CloudStorageCalculator {
    public static void main(String[] args) {
        ...
        double baseCost = 0;
        if (storage <= 100)
            baseCost = storage * 0.05;
        else if (storage <= 500)
            baseCost = (100 * 0.05) + (storage - 100) * 0.04;
        else if (storage <= 2000)
            baseCost = (100 * 0.05) + (400 * 0.04) + (storage - 500) * 0.03;
        else
            baseCost = (100 * 0.05) + (400 * 0.04) + (1500 * 0.03) + (storage - 2000) * 0.02;
        double perUserFee;
        if (users <= 10) perUserFee = users * 5;
        else if (users <= 50) perUserFee = users * 4;
        else perUserFee = users * 3;
        double backupPremium = 0;
        if (backup.equals("Hourly")) backupPremium = 15 + (0.5 * users);
        else if (backup.equals("Real-Time")) backupPremium = 50 + (1 * users);
        double supportFee = 0;
        ...
        double monthly = baseCost + perUserFee + backupPremium + supportFee;
        ...
        System.out.println("Base Storage Cost: $" + baseCost);
